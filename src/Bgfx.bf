...
namespace Bgfx
{
	public static class bgfx
	{
		...
		{
			public uint16 idx = UInt16.MaxValue;
		...
		/// <param name="_normalized">When using fixed point AttribType (f.e. Uint8) value will be normalized for vertex
		// shader usage. When normalized is set to true, AttribType::Uint8 value in range 0-255 will be in range 0.0-1.0
		// in vertex shader.</param> 	<param name="_asInt">Packaging rule for vertexPack, vertexUnpack, and
		// vertexConvert for AttribType::Uint8 and AttribType::Int16. Unpacking code must be implemented inside vertex
		// shader.</param>
		...
		/// </summary>
		///
		/// <param name="_output">Welded vertices remapping table. The size of buffer must be the same as number of
		// vertices.</param> 	<param name="_layout">Vertex stream layout.</param> 	<param name="_data">Vertex
		// stream.</param> 	<param name="_num">Number of vertices in vertex stream.</param> 	<param
		// name="_index32">Set to `true` if input indices are 32-bit.</param> 	<param name="_epsilon">Error tolerance
		// for vertex position comparison.</param>
		...
		/// <param name="_dst">Destination index buffer. If this argument is NULL function will return number of indices
		// after conversion.</param> 	<param name="_dstSize">Destination index buffer in bytes. It must be large
		// enough to contain output indices. If destination size is insufficient index buffer will be truncated.</param>
		//  <param name="_indices">Source indices.</param> 	<param name="_numIndices">Number of input indices.</param>  
		...
		/// <param name="_dstSize">Destination index buffer in bytes. It must be large enough to contain output indices.
		// If destination size is insufficient index buffer will be truncated.</param> 	<param name="_dir">Direction
		// (vector must be normalized).</param> 	<param name="_pos">Position.</param> 	<param
		// name="_vertices">Pointer to first vertex represented as float x, y, z. Must contain at least number of
		// vertices referencende by index buffer.</param> 	<param name="_stride">Vertex stride.</param> 	<param
		// name="_indices">Source indices.</param> 	<param name="_numIndices">Number of input indices.</param> 
		// <param name="_index32">Set to `true` if input indices are 32-bit.</param>
		...
		public static extern void* get_renderer_name(RendererType _type);
		...
		/// <param name="_flags">See: `BGFX_RESET_*` for more info.   - `BGFX_RESET_NONE` - No reset flags.   -
		...
		// `BGFX_RESET_CAPTURE` - Begin screen capture.   - `BGFX_RESET_FLUSH_AFTER_RENDER` - Flush rendering after
		// submitting to GPU.   - `BGFX_RESET_FLIP_AFTER_RENDER` - This flag  specifies where flip     occurs. Default
		// behaviour is that flip occurs before rendering new     frame. This flag only has effect when
		// `BGFX_CONFIG_MULTITHREADED=0`.   - `BGFX_RESET_SRGB_BACKBUFFER` - Enable sRGB backbuffer.</param> 	<param
		// name="_format">Texture format. See: `TextureFormat::Enum`.</param>
		[LinkName("bgfx_reset")]
		...
		/// </summary>
		///
		/// <param name="_debug">Available flags:   - `BGFX_DEBUG_IFH` - Infinitely fast hardware. When this flag is set
		//     all rendering calls will be skipped. This is useful when profiling     to quickly assess potential
		// bottlenecks between CPU and GPU.   - `BGFX_DEBUG_PROFILER` - Enable profiler.   - `BGFX_DEBUG_STATS` -
		// Display internal statistics.   - `BGFX_DEBUG_TEXT` - Display debug text.   - `BGFX_DEBUG_WIREFRAME` -
		// Wireframe rendering. All rendering     primitives will be rendered as lines.</param>
		...
		/// <param name="_attr">Color palette. Where top 4-bits represent index of background, and bottom 4-bits
		// represent foreground color from standard VGA text palette (ANSI escape codes).</param> 	<param
		// name="_format">`printf` style format.</param>
		[LinkName("bgfx_dbg_text_printf")]
		public static extern void dbg_text_printf(uint16 _x, uint16 _y, uint8 _attr, char8* _format, char8* args);

		/// <summary>
		/// Print formatted data from variable argument list to internal debug text character-buffer (VGA-compatible
		// text mode). 	</summary>  	<param name="_x">Position x from the left corner of the window.</param> 
		// <param name="_y">Position y from the top corner of the window.</param> 	<param name="_attr">Color palette.
		// Where top 4-bits represent index of background, and bottom 4-bits represent foreground color from standard
		// VGA text palette (ANSI escape codes).</param> 	<param name="_format">`printf` style format.</param> 	<param
		// name="_argList">Variable arguments list for format string.</param>
		...
		/// <param name="_flags">Buffer creation flags.   - `BGFX_BUFFER_NONE` - No flags.   -
		// `BGFX_BUFFER_COMPUTE_READ` - Buffer will be read from by compute shader.   - `BGFX_BUFFER_COMPUTE_WRITE` -
		// Buffer will be written into by compute shader. When buffer       is created with `BGFX_BUFFER_COMPUTE_WRITE`
		// flag it cannot be updated from CPU.   - `BGFX_BUFFER_COMPUTE_READ_WRITE` - Buffer will be used for read/write
		// by compute shader.   - `BGFX_BUFFER_ALLOW_RESIZE` - Buffer will resize on buffer update if a different amount
		// of       data is passed. If this flag is not specified, and more data is passed on update, the buffer      
		// will be trimmed to fit the existing buffer size. This flag has effect only on dynamic       buffers.   -
		// `BGFX_BUFFER_INDEX32` - Buffer is using 32-bit indices. This flag has effect only on       index
		// buffers.</param>
		...
		/// <param name="_len">Static index buffer name length (if length is INT32_MAX, it's expected that _name is zero
		// terminated string.</param>
		...
		/// <param name="_layout">Vertex layout.</param>
		/// <param name="_flags">Buffer creation flags.  - `BGFX_BUFFER_NONE` - No flags.  - `BGFX_BUFFER_COMPUTE_READ`
		// - Buffer will be read from by compute shader.  - `BGFX_BUFFER_COMPUTE_WRITE` - Buffer will be written into by
		// compute shader. When buffer      is created with `BGFX_BUFFER_COMPUTE_WRITE` flag it cannot be updated from
		// CPU.  - `BGFX_BUFFER_COMPUTE_READ_WRITE` - Buffer will be used for read/write by compute shader.  -
		// `BGFX_BUFFER_ALLOW_RESIZE` - Buffer will resize on buffer update if a different amount of      data is
		// passed. If this flag is not specified, and more data is passed on update, the buffer      will be trimmed to
		// fit the existing buffer size. This flag has effect only on dynamic buffers.  - `BGFX_BUFFER_INDEX32` - Buffer
		// is using 32-bit indices. This flag has effect only on index buffers.</param>
		...
		/// <param name="_len">Static vertex buffer name length (if length is INT32_MAX, it's expected that _name is
		// zero terminated string.</param>
		...
		/// </summary>
		///
		/// <param name="_tib">TransientIndexBuffer structure is filled and is valid for the duration of frame, and it
		// can be reused for multiple draw calls.</param> 	<param name="_num">Number of indices to allocate.</param>
		...
		/// </summary>
		///
		/// <param name="_tvb">TransientVertexBuffer structure is filled and is valid for the duration of frame, and it
		// can be reused for multiple draw calls.</param> 	<param name="_num">Number of vertices to allocate.</param> 
		// <param name="_layout">Vertex layout.</param>
		...
		///
		/// <param name="_tvb">TransientVertexBuffer structure is filled and is valid for the duration of frame, and it
		// can be reused for multiple draw calls.</param> 	<param name="_layout">Vertex layout.</param> 	<param
		// name="_numVertices">Number of vertices to allocate.</param> 	<param name="_tib">TransientIndexBuffer
		// structure is filled and is valid for the duration of frame, and it can be reused for multiple draw
		// calls.</param> 	<param name="_numIndices">Number of indices to allocate.</param>
		...
		/// </summary>
		///
		/// <param name="_idb">InstanceDataBuffer structure is filled and is valid for duration of frame, and it can be
		// reused for multiple draw calls.</param> 	<param name="_num">Number of instances.</param> 	<param
		// name="_stride">Instance stride. Must be multiple of 16.</param>
		...
		/// <param name="_len">Shader name length (if length is INT32_MAX, it's expected that _name is zero terminated
		// string).</param>
		...
		/// <param name="_flags">Texture creation (see `BGFX_TEXTURE_*`.), and sampler (see `BGFX_SAMPLER_*`) flags.
		// Default texture sampling mode is linear, and wrap mode is repeat. - `BGFX_SAMPLER_[U/V/W]_[MIRROR/CLAMP]` -
		// Mirror or clamp to edge wrap   mode. - `BGFX_SAMPLER_[MIN/MAG/MIP]_[POINT/ANISOTROPIC]` - Point or
		// anisotropic   sampling.</param> 	<param name="_skip">Skip top level mips when parsing texture.</param> 
		...
		/// <param name="_height">Height.</param>
		/// <param name="_hasMips">Indicates that texture contains full mip-map chain.</param>
		/// <param name="_numLayers">Number of layers in texture array. Must be 1 if caps `BGFX_CAPS_TEXTURE_2D_ARRAY`
		// flag is not set.</param> 	<param name="_format">Texture format. See: `TextureFormat::Enum`.</param> 	<param
		// name="_flags">Texture creation (see `BGFX_TEXTURE_*`.), and sampler (see `BGFX_SAMPLER_*`) flags. Default
		// texture sampling mode is linear, and wrap mode is repeat. - `BGFX_SAMPLER_[U/V/W]_[MIRROR/CLAMP]` - Mirror or
		// clamp to edge wrap   mode. - `BGFX_SAMPLER_[MIN/MAG/MIP]_[POINT/ANISOTROPIC]` - Point or anisotropic  
		// sampling.</param> 	<param name="_mem">Texture data. If `_mem` is non-NULL, created texture will be
		// immutable. If `_mem` is NULL content of the texture is uninitialized. When `_numLayers` is more than 1,
		// expected memory layout is texture and all mips together for each array element.</param>
		...
		// Default texture sampling mode is linear, and wrap mode is repeat. - `BGFX_SAMPLER_[U/V/W]_[MIRROR/CLAMP]` -
		// Mirror or clamp to edge wrap   mode. - `BGFX_SAMPLER_[MIN/MAG/MIP]_[POINT/ANISOTROPIC]` - Point or
		// anisotropic   sampling.</param> 	<param name="_mem">Texture data. If `_mem` is non-NULL, created texture
		// will be immutable. If `_mem` is NULL content of the texture is uninitialized. When `_numLayers` is more than
		// 1, expected memory layout is texture and all mips together for each array element.</param>
		...
		/// <param name="_pitch">Pitch of input image (bytes). When _pitch is set to UINT16_MAX, it will be calculated
		// internally based on _width.</param>
		...
		/// <param name="_handle">Texture handle.</param>
		/// <param name="_layer">Layer in texture array.</param>
		/// <param name="_side">Cubemap side `BGFX_CUBE_MAP_<POSITIVE or NEGATIVE>_<X, Y or Z>`,   where 0 is +X, 1 is
		// -X, 2 is +Y, 3 is -Y, 4 is +Z, and 5 is -Z.                  +----------+                  |-z       2|      
		//            | ^  +y    |                  | |        |    Unfolded cube:                  | +---->+x |      
		// +----------+----------+----------+----------+       |+y       1|+y       4|+y       0|+y       5|       | ^ 
		// -x    | ^  +z    | ^  +x    | ^  -z    |       | |        | |        | |        | |        |       | +---->+z
		// | +---->+x | +---->-z | +---->-x |       +----------+----------+----------+----------+                  |+z  
		//     3|                  | ^  -y    |                  | |        |                  | +---->+x |             
		//     +----------+</param> 	<param name="_mip">Mip level.</param> 	<param name="_x">X offset in
		// texture.</param> 	<param name="_y">Y offset in texture.</param> 	<param name="_width">Width of texture
		// block.</param> 	<param name="_height">Height of texture block.</param> 	<param name="_mem">Texture
		// update data.</param> 	<param name="_pitch">Pitch of input image (bytes). When _pitch is set to UINT16_MAX,
		// it will be calculated internally based on _width.</param>
		...
		/// <param name="_len">Texture name length (if length is INT32_MAX, it's expected that _name is zero terminated
		// string.</param>
		...
		/// <param name="_format">Texture format. See: `TextureFormat::Enum`.</param>
		/// <param name="_textureFlags">Texture creation (see `BGFX_TEXTURE_*`.), and sampler (see `BGFX_SAMPLER_*`)
		// flags. Default texture sampling mode is linear, and wrap mode is repeat. -
		// `BGFX_SAMPLER_[U/V/W]_[MIRROR/CLAMP]` - Mirror or clamp to edge wrap   mode. -
		// `BGFX_SAMPLER_[MIN/MAG/MIP]_[POINT/ANISOTROPIC]` - Point or anisotropic   sampling.</param>
		...
		/// <param name="_len">Frame buffer name length (if length is INT32_MAX, it's expected that _name is zero
		// terminated string.</param>
		...
		/// <param name="_result">Number of pixels that passed test. This argument can be `NULL` if result of occlusion
		// query is not needed.</param>
		...
		/// <param name="_x">Position x from the left corner of the window.</param>
		/// <param name="_y">Position y from the top corner of the window.</param>
		/// <param name="_ratio">Width and height will be set in respect to back-buffer size. See:
		// `BackbufferRatio::Enum`.</param>
		...
		///
		/// <param name="_id">View id.</param>
		/// <param name="_flags">Clear flags. Use `BGFX_CLEAR_NONE` to remove any clear operation. See:
		// `BGFX_CLEAR_*`.</param> 	<param name="_rgba">Color clear value.</param> 	<param name="_depth">Depth
		// clear value.</param> 	<param name="_stencil">Stencil clear value.</param>
		...
		/// <param name="_id">View id.</param>
		/// <param name="_flags">Clear flags. Use `BGFX_CLEAR_NONE` to remove any clear operation. See:
		// `BGFX_CLEAR_*`.</param> 	<param name="_depth">Depth clear value.</param> 	<param
		// name="_stencil">Stencil clear value.</param> 	<param name="_c0">Palette index for frame buffer attachment
		// 0.</param> 	<param name="_c1">Palette index for frame buffer attachment 1.</param> 	<param
		// name="_c2">Palette index for frame buffer attachment 2.</param> 	<param name="_c3">Palette index for
		// frame buffer attachment 3.</param> 	<param name="_c4">Palette index for frame buffer attachment 4.</param> 
		// <param name="_c5">Palette index for frame buffer attachment 5.</param> 	<param name="_c6">Palette index for
		// frame buffer attachment 6.</param> 	<param name="_c7">Palette index for frame buffer attachment 7.</param>
		...
		///
		/// <param name="_id">View id.</param>
		/// <param name="_handle">Frame buffer handle. Passing `BGFX_INVALID_HANDLE` as frame buffer handle will draw
		// primitives from this view into default back buffer.</param>
		...
		/// </summary>
		///
		/// <param name="_state">State flags. Default state for primitive type is   triangles. See:
		// `BGFX_STATE_DEFAULT`.   - `BGFX_STATE_DEPTH_TEST_*` - Depth test function.   - `BGFX_STATE_BLEND_*` - See
		// remark 1 about BGFX_STATE_BLEND_FUNC.   - `BGFX_STATE_BLEND_EQUATION_*` - See remark 2.   -
		// `BGFX_STATE_CULL_*` - Backface culling mode.   - `BGFX_STATE_WRITE_*` - Enable R, G, B, A or Z write.   -
		// `BGFX_STATE_MSAA` - Enable hardware multisample antialiasing.   - `BGFX_STATE_PT_[TRISTRIP/LINES/POINTS]` -
		// Primitive type.</param> 	<param name="_rgba">Sets blend factor used by `BGFX_STATE_BLEND_FACTOR` and  
		// `BGFX_STATE_BLEND_INV_FACTOR` blend modes.</param>
		...
		/// <param name="_bstencil">Back stencil state. If back is set to `BGFX_STENCIL_NONE` _fstencil is applied to
		// both front and back facing primitives.</param>
		...
		/// <param name="_num">Number of elements. Passing `UINT16_MAX` will use the _num passed on uniform
		// creation.</param>
		...
		/// <param name="_layoutHandle">Vertex layout for aliasing vertex buffer. If invalid handle is used, vertex
		// layout used for creation of vertex buffer will be used.</param>
		[LinkName("bgfx_encoder_set_vertex_buffer")]
		public static extern void encoder_set_vertex_buffer(Encoder* _this, uint8 _stream, VertexBufferHandle _handle, uint32 _startVertex, uint32 _numVertices, VertexLayoutHandle _layoutHandle);
		...
		public static extern void encoder_set_dynamic_vertex_buffer(Encoder* _this, uint8 _stream, DynamicVertexBufferHandle _handle, uint32 _startVertex, uint32 _numVertices, VertexLayoutHandle _layoutHandle);
		...
		public static extern void encoder_set_transient_vertex_buffer(Encoder* _this, uint8 _stream, TransientVertexBuffer* _tvb, uint32 _startVertex, uint32 _numVertices, VertexLayoutHandle _layoutHandle);
		...
		/// <param name="_handle">Texture handle.</param>
		/// <param name="_flags">Texture sampling mode. Default value UINT32_MAX uses   texture sampling settings from
		// the texture.   - `BGFX_SAMPLER_[U/V/W]_[MIRROR/CLAMP]` - Mirror or clamp to edge wrap     mode.   -
		// `BGFX_SAMPLER_[MIN/MAG/MIP]_[POINT/ANISOTROPIC]` - Point or anisotropic     sampling.</param>
		...
		/// <param name="_dstZ">If texture is 2D this argument should be 0. If destination texture is cube this argument
		// represents destination texture cube face. For 3D texture this argument represents destination texture Z
		// position.</param> 	<param name="_src">Source texture handle.</param> 	<param name="_srcMip">Source texture
		// mip level.</param> 	<param name="_srcX">Source texture X position.</param> 	<param name="_srcY">Source
		// texture Y position.</param> 	<param name="_srcZ">If texture is 2D this argument should be 0. If source
		// texture is cube this argument represents source texture cube face. For 3D texture this argument represents
		// source texture Z position.</param> 	<param name="_width">Width of region.</param> 	<param
		// name="_height">Height of region.</param> 	<param name="_depth">If texture is 3D this argument represents
		// depth of region, otherwise it's unused.</param>
		...
		/// </summary>
		///
		/// <param name="_handle">Frame buffer handle. If handle is `BGFX_INVALID_HANDLE` request will be made for main
		// window back buffer.</param> 	<param name="_filePath">Will be passed to `bgfx::CallbackI::screenShot`
		// callback.</param>
		...
		public static StateFlags blend_function_separate(StateFlags _srcRGB, StateFlags _dstRGB, StateFlags _srcA, StateFlags _dstA)
		{
			return (StateFlags)(uint64(0) | (((uint64)(_srcRGB) | ((uint64)(_dstRGB) << 4))) | (((uint64)(_srcA) | ((uint64)(_dstA) << 4)) << 8));
		}

		public static StateFlags blend_function(StateFlags _srcRGB, StateFlags _dstRGB)
		{
